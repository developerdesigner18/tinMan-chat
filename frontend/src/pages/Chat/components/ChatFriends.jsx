...
const ChatFriends = ({ className }) => {
  ...
  return (
    <div
      className={`flex flex-col w-full h-full divide-y divide-light-secondary-200 dark:divide-secondary-200 ${className}`}
    >
      <div className="px-4 py-[9px] dark:py-[10px]">
        <div className="transition-none p-2 rounded-md border border-light-secondary dark:border-0 dark:bg-dark-secondary flex justify-center items-center gap-2">
          ...
          <input
            type="text"
            className="transition-none w-full text-sm sm:text-[1rem] border-none outline-none bg-inherit text-light-primary placeholder-dark-primary dark:placeholder-light-primary"
            placeholder="Search..."
            value={search}
