...
const ChatMessages = () => {
   ...
   const { userId, selectedChat, messages, messagesLoading, messageText, sendMessageLoading } = useSelector(state => state.chatReducer);
   ...
   return (
      <div className="w-full h-full flex flex-col divide-y divide-secondary-200">
         ...
         <div className="h-auto bg-primary p-2 mt-auto flex justify-center items-center gap-2">
            <AutoResizeableTextarea
               className="p-2 bg-transparent text-sm sm:text-[1rem] text-light-primary placeholder-light-primary rounded-lg"
            ...
            <button
               ...
               onClick={sendMessage}
            >
               <SendOutlined className="leading-[0] text-light-primary text-2xl cursor-pointer translate-x-0 group-hover:translate-x-1" />
            </button>
         </div>
      </div>
   )
